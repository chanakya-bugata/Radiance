...
class Question2ViewController: UIViewController {
    ...
    @objc func concernButtonTapped(_ sender: UIButton) {
        if sender.backgroundColor == customPink {
            ...
            if let title = sender.titleLabel?.text, let index = User.shared.skinConcerns.firstIndex(of: title) {
                User.shared.skinConcerns.remove(at: index)
            }
        } else {
            sender.backgroundColor = customPink
            sender.setTitleColor(.white, for: .normal)
            if let title = sender.titleLabel?.text, !User.shared.skinConcerns.contains(title) {
